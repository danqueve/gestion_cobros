...
// CORRECCIÓN DE CONSISTENCIA #1:
//   sort_link_reporte() eliminada; se usa sort_link() de config.php
...
// Parámetros extra para preservar el limit al hacer clic en columnas
$sort_extra = array_filter(['limit' => $limit !== 15 ? $limit : null]);
...
try {
    ...
    $sql = "SELECT
                ...
                cr.monto_cuota, cr.total_cuotas, cr.cuotas_pagadas,
...
<div class="bg-gray-800 p-6 rounded-lg border border-gray-700">
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6">
        ...
        <a href="views/exportar_excel.php"
           class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-md mt-4 sm:mt-0">
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-700">
            <thead class="table-header-custom">
                <tr>
                    <!-- CORRECCIÓN #1: sort_link() unificada con page='reporte_general' -->
                    <th class="px-4 py-3 text-left   text-xs font-medium text-gray-300 uppercase tracking-wider">
                        <?= sort_link('nombre',         'Cliente',      $sort_by, $sort_order, 'reporte_general', $sort_extra) ?>
                    </th>
                    <th class="px-4 py-3 text-left   text-xs font-medium text-gray-300 uppercase tracking-wider">
                        Teléfono
                    </th>
                    <th class="px-4 py-3 text-center  text-xs font-medium text-gray-300 uppercase tracking-wider">
                        <?= sort_link('zona',           'Zona',         $sort_by, $sort_order, 'reporte_general', $sort_extra) ?>
                    </th>
                    <th class="px-4 py-3 text-center  text-xs font-medium text-gray-300 uppercase tracking-wider">
                        Monto Cuota
                    </th>
                    ...
                    <th class="px-4 py-3 text-center  text-xs font-medium text-gray-300 uppercase tracking-wider">
                        <?= sort_link('estado_credito', 'Estado',       $sort_by, $sort_order, 'reporte_general', $sort_extra) ?>
                    </th>
                    <th class="px-4 py-3 text-center  text-xs font-medium text-gray-300 uppercase tracking-wider">
                        <?= sort_link('ultimo_pago',    'Último Pago',  $sort_by, $sort_order, 'reporte_general', $sort_extra) ?>
                    </th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-700">
                ...
                <?php else: ?>
                    ...
                    <tr class="table-row-dark hover:bg-gray-700/50">
                        <td class="px-4 py-3 whitespace-nowrap font-medium text-gray-100">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-center text-gray-300">
                            <?= $row['monto_cuota'] ? formatCurrency($row['monto_cuota']) : 'N/A' ?>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap text-center text-gray-300">
                            <?= $row['credito_id']
                                ? ($row['cuotas_pagadas'] . ' / ' . $row['total_cuotas'])
                                : 'N/A' ?>
                        </td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-center text-gray-300">
                            <?= $row['ultimo_pago']
                                ? (new DateTime($row['ultimo_pago']))->format('d/m/Y')
                                : 'N/A' ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
    ...
    <div class="mt-6 flex justify-between items-center text-sm text-gray-400">
        <div>
            Mostrando <?= $offset + 1 ?> a <?= min($offset + $limit, $total_records) ?>
            de <?= $total_records ?> registros.
        </div>
        <div class="flex items-center gap-2">
            <a href="?page=reporte_general&p=<?= $page_num - 1 ?>&limit=<?= $limit ?>&sort_by=<?= $sort_by ?>&sort_order=<?= $sort_order ?>"
               class="<?= $page_num <= 1 ? 'pointer-events-none text-gray-600' : 'text-blue-400 hover:text-blue-300' ?>">
                <i class="fas fa-chevron-left"></i> Anterior
            </a>
            <div class="flex gap-1">
                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                    <a href="?page=reporte_general&p=<?= $i ?>&limit=<?= $limit ?>&sort_by=<?= $sort_by ?>&sort_order=<?= $sort_order ?>"
